...
public class forgot2 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet forgot2</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet forgot2 at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h4>Me diste un email: " + email + "</h4>");
            out.println("<h4>Una pregunta: " + pregunta + "</h4>");
            out.println("<h4>Y una respuesta: " + respuesta + "</h4>");
            out.println("</body>");
            out.println("</html>");
    //DB CONNECTION
    try{
    ...
    Connection cn = DriverManager.getConnection("jdbc:mysql://localhost:3306/filereader", "root", "");
     ...
     out.println("<form action='forgot' method='POST'>");
     out.println("<input type=hidden name=email value='"+email+"'>");
     out.println("<input type='submit' value='Intentalo Otra Vez'>");
     out.println("</form>");
     }       
     }  
    }catch(Exception ex){
